...
def mine_block(k, prev_hash, rand_lines):
    ...
    rand_lines_bytes = ''.join(rand_lines).encode('utf-8')
    target = '0' * k

    nonce = 0
    while True:
        nonce_bytes = nonce.to_bytes((nonce.bit_length() + 7) // 8, 'big')
        hash_result = hashlib.sha256(prev_hash + rand_lines_bytes + nonce_bytes).digest()
        hash_bin = bin(int.from_bytes(hash_result, 'big'))[2:].zfill(256)
        if hash_bin[-k:] == target:
...
if __name__ == '__main__':
    ...
    nonce = mine_block(diff,os.urandom(32), rand_lines)
    print(nonce)
